...
class LabelTool():
    def __init__(self, master):
        ...
        self.cla_can_temp = []
        self.classcandidate_filename = 'obj.names'
    ...
    def saveImage(self):
        ...
        with open(self.labelfilename, 'w') as f:
            for bbox in self.bboxList:
                f.write("{} {} {} {} {}\n".format(contents.index(bbox[4]),int(int(bbox[0])*self.factor)/self.img.size[0],
                                                int(int(bbox[1])*self.factor)/self.img.size[1],
                                                int(int(bbox[2])*self.factor)/self.img.size[0],
                                                int(int(bbox[3])*self.factor)/self.img.size[1]))
